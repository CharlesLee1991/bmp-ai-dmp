...
const C = { card: "#ffffff", border: "#e2e8f0", text: "#1a202c", sub: "#718096", accent: "#0d9488" };
...
export default function BehaviorPlaceholder({ behavior }: { behavior: string }) {
  ...
  if (!m) return null;
  return (
    <div style={{ margin: "40px 28px", padding: "48px 32px", border: "2px dashed #cbd5e1", borderRadius: 16, background: C.card, textAlign: "center" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", maxWidth: 680, margin: "0 auto" }}>
        {m.dims.map((d, i) => (
          <div key={i} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 16px", fontSize: 12, minWidth: 160, textAlign: "left", background: "#fafbfc" }}>
