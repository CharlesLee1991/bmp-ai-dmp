...
import { Clock, Zap, Bot, Target, FlaskConical, Users, AlertTriangle, ClipboardList, type LucideIcon } from "lucide-react";
...
const EXAMPLES: { group: string; icon: LucideIcon; desc: string; items: string[] }[] = [
  {
    group: "행동 타겟팅", icon: Clock, desc: "활동 시간·빈도·강도 기반",
    items: [
  ...
  {
    group: "전환·반응 (G6)", icon: Zap, desc: "실제 전환 이력 기반 — 반응하는 사람",
    items: [
  ...
  {
    group: "AI 모델 결합", icon: Bot, desc: "룩어라이크·반응예측 오디언스와 교차",
    items: [
  ...
  {
    group: "광고주 시나리오 (AX)", icon: Target, desc: "캠페인 상황을 그대로 문장으로",
    items: [
...
const STATUS_META: Record<string, { label: string; color: string }> = {
  pending: { label: "승인 대기", color: "var(--badge-warning-fg)" },
  executed: { label: "생성됨", color: "var(--badge-success-fg)" },
  approved: { label: "실행 중", color: "var(--badge-info-fg)" },
...
export default function AiExploreTab() {
  ...
  return (
    <div style={{ padding: "20px 28px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12, marginBottom: 18 }}>
        {EXAMPLES.map(g => (
          <div key={g.group} style={{ padding: 14, borderRadius: 12, background: "var(--card)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}><g.icon size={14} style={{ verticalAlign: "-2px", marginRight: 6, color: "var(--accent)" }} />{g.group}</div>
            <div style={{ fontSize: 10, color: "var(--neutral)", marginBottom: 8 }}>{g.desc}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {g.items.map(it => (
                <button key={it} onClick={() => { setText(it); create(it); }}
                  style={{ textAlign: "left", padding: "7px 10px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--badge-sky-bg)", fontSize: 11.5, color: "var(--badge-sky-fg)", cursor: "pointer", lineHeight: 1.4 }}>
                  {it}
                </button>
              ))}
            </div>
      ...
      {/* ② 탐색 입력 + 결과 */}
      <div style={{ padding: 18, borderRadius: 12, background: "var(--bg-elevated)", border: "1px solid var(--border-soft)", marginBottom: 18 }}>
        ...
        {r?.request_id && (
          <div style={{ marginTop: 14 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 8, fontSize: 12 }}>
              ...
              {r.est_rows != null && <span style={{ color: "var(--badge-sky-fg)", fontWeight: 700 }}><Users size={13} style={{ verticalAlign: "-2px", marginRight: 6, color: "var(--accent)" }} />예상 {fmtRows(r.est_rows)}</span>}
            ...
            <tbody>
              {history.map(h => (
                <tr key={h.id} style={{ borderBottom: "1px solid var(--bg-elevated)" }}>
                  ...
                  <td style={{ padding: "7px 8px", fontFamily: "monospace", fontSize: 11, color: "var(--badge-success-fg)" }}>{h.result_table || "—"}</td>
                  ...
                  <td style={{ padding: "7px 8px", whiteSpace: "nowrap" }}>
                    {h.status === "pending" && (
                      <>
                        ...
                        <button disabled={acting} onClick={() => act("reject", h.id)} style={{ fontSize: 11, padding: "4px 10px", marginRight: 4, borderRadius: 6, border: "1px solid var(--border)", background: "var(--card)", color: "var(--badge-danger-fg)", cursor: "pointer", fontWeight: 700 }}>거절</button>
                      </>
                    )}
                    {h.status !== "pending" && (
                      <button disabled={loading} onClick={() => rerun(h.query_text)} title="같은 문장으로 다시 탐색(SQL 재생성)" style={{ fontSize: 11, padding: "4px 10px", marginRight: 4, borderRadius: 6, border: "1px solid var(--border)", background: "var(--card)", color: "var(--badge-info-fg)", cursor: "pointer", fontWeight: 700 }}>↻ 재실행</button>
                    )}
