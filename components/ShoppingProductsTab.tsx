      ...
      {subView === "nonfood" && (<>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ background: P.card, borderRadius: 12, padding: 18, border: `1px solid ${P.border}` }}>
            ...
            {platforms.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{platforms.map((p, i) => { const pct = totalPlatformCnt > 0 ? (p.cnt / totalPlatformCnt * 100) : 0; return (<div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 11, fontWeight: 600, width: 100, flexShrink: 0 }}>{p.platform_name}</span><div style={{ flex: 1, height: 22, background: "var(--border)", borderRadius: 4, overflow: "hidden", position: "relative" }}><div style={{ height: "100%", background: CHART_COLORS[i], borderRadius: 4, width: `${pct}%`, transition: "width .4s", display: "flex", alignItems: "center", paddingLeft: 6 }}>{pct > 15 && <span style={{ fontSize: 9, color: "#fff", fontWeight: 700 }}>{fmt(p.cnt)}건</span>}</div></div><span style={{ fontSize: 10, fontWeight: 700, color: P.accent, width: 44, textAlign: "right" }}>{pct.toFixed(1)}%</span><span style={{ fontSize: 10, color: P.sub, width: 50, textAlign: "right" }}>{fmtAmt(p.revenue)}</span></div>); })}</div>) : <div style={{ textAlign: "center", padding: 40, color: P.sub, fontSize: 12 }}>—</div>}
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          <div style={{ background: P.card, borderRadius: 12, padding: 18, border: `1px solid ${P.border}` }}>
        ...
        <div style={{ background: P.card, borderRadius: 16, padding: 0, border: `1px solid ${P.border}`, width: 720, maxWidth: "94vw", maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.15)" }} onClick={e => e.stopPropagation()}>
          ...
          {detail && (<div style={{ padding: "16px 24px 24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
              ...
              <div><div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8 }}><CreditCard size={13} strokeWidth={2} style={{ verticalAlign: "-2px", marginRight: 6, color: P.accent }} />제휴처</div><div style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 8 }}>{(detail.by_platform as any[]).map((p: any, i: number) => { const total = (detail.by_platform as any[]).reduce((s: number, x: any) => s + x.cnt, 0); const pct = total > 0 ? (p.cnt / total * 100) : 0; return (<div key={i}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginBottom: 3 }}><span style={{ fontWeight: 600 }}>{p.label}</span><span style={{ color: P.sub }}>{fmt(p.cnt)}건 ({pct.toFixed(0)}%)</span></div><div style={{ height: 6, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", background: CHART_COLORS[i], borderRadius: 3, width: `${pct}%`, transition: "width .3s" }} /></div></div>); })}</div></div>
            </div>
          </div>)}
        </div>
      </div>)}
