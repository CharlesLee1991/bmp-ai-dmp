...
function categorize(title: string): string {
  ...
  if (/생수|샘물|삼다수|탐사수|스파클|주스|콜라|사이다|이온음료|탄산|음료|게토레이|파워에이드|비타500|에너지드링크|식혜|수정과|매실|포카리/.test(t)) return "음료/생수";
  if (/커피|맥심|카누|네스카페|아메리카노|라떼|에스프레소|콜드브루|믹스커피/.test(t)) return "커피";
  if (/과자|빼빼로|오예스|초코파이|칩$|새우깡|감자깡|포카칩|프링글스|꼬깔콘|다이제|쿠키|비스킷|젤리|사탕|껌|초콜릿|파슐랭|파슬링|웨하스|크래커|스낵|캔디|마카롱|케이크|파이|타르트|도넛|빵$|모닝빵|식빵/.test(t)) return "간식/과자";
  if (/햇반|두부|어묵|김$|김밥|소시지|햄$|베이컨|만두|떡볶이|떡$|오뎅|순대|핫도그|냉동|통조림|참치캔|스팸|맛살|크래미|피자|치킨|돈까스|탕수육|볶음밥/.test(t)) return "가공식품";
  if (/계란|대란|중란|소란|달걀|왕란|특란|반숙란|메추리|훈제란|구운란/.test(t)) return "달걀";
  ...
  if (/수세미|세제|화장지|세탁|샴푸|린스|바디워시|칫솔|치약|물티슈|휴지|키친타올|구강청결제|가그린|리스테린|핸드워시|손세정/.test(t)) return "생활용품";
  return "기타";
}
