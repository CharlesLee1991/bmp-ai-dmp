...
export async function GET(req: NextRequest) {
  ...
  try {
    const major = req.nextUrl.searchParams.get("major");
    const middle = req.nextUrl.searchParams.get("middle");
    const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/dmp_options_subcategory`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ p_major: major, p_middle: middle }),
    });
